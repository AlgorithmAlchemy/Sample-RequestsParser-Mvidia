...
connect = db.cursor()
connect.execute('''CREATE TABLE IF NOT EXISTS xboxe_id (
...
connect.execute('''CREATE TABLE IF NOT EXISTS playstation_id_ps5 (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  item_name TEXT,
  item_id TEXT,
  UNIQUE (item_id)
...
connect.execute('''CREATE TABLE IF NOT EXISTS playstation_id_ps4 (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  item_name TEXT,
  item_id TEXT,
  UNIQUE (item_id)
...
connect.execute('''CREATE TABLE IF NOT EXISTS nintendo_id (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  item_name TEXT,
  item_id TEXT,
  UNIQUE (item_id)
...
connect.execute('''CREATE TABLE IF NOT EXISTS items (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  item_name TEXT,
  item_id TEXT,
  tovar_price TEXT,
...
def get_games_1c():
    while True:
        try:
            games = []
            r = requests.get('ТУТ ПОЛУЧАЛ ЭТОТ СПИСОК КОТОРЫЙ Я ТЕБЕ СКИНУЛ')
...
def create_exel():
    ...
    for itr in cursor:
        ...
        if parse_price is not None:
            ...
            # parse_full_list.append({"parse_original_name": parse_original_name, "name": parse_name, "type": game_type,
            #                         "price": parse_price})
            parse_full_list[parse_name] = {"parse_original_name": parse_name, "type": game_type,
                                           "price": parse_price}
            # parse_full_list[count] = {"parse_original_name": parse_original_name, "name": parse_name, "type": game_type,
            #                         "price": parse_price}

            count += 1
            # parse_list.append(parse_name)
    high_key = list(parse_full_list.keys())
    high_key = high_key[-1]

    json_game = get_games_1c()  # выгружаем наш жсон
    json_double = []
    count = 0
    for jsg in json_game:
        ...
        for match in parse_full_list:  # сравниваем базу данных скл и жсон
            if json_game_name in match:
                ...
                # print(parse_full_list[match]['price'])
        count += 1
        # matches = [match for match in parse_full_list if json_game_name in match]
        if not matches:
            continue
